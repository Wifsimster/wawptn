...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable('personas', (table) => {
    ...
    table.text('system_prompt_overlay').notNullable()
    table.jsonb('friday_messages').notNullable()
    table.jsonb('weekday_messages').notNullable()
    table.jsonb('back_online_messages').notNullable()
    table.text('empty_mention_reply').notNullable()
    table.text('intro_message').notNullable()
    table.integer('embed_color').notNullable()
  ...
  // Seed the 7 existing personas with is_default = true
  await knex('personas').insert([
    {
      id: 'pote-sarcastique',
      name: 'Le Pote Sarcastique',
      system_prompt_overlay: `Ta personnalité :
...
- Tu parles normalement, comme un pote, sans abuser du jargon gamer`,
      friday_messages: JSON.stringify([
      ...
      weekday_messages: JSON.stringify([
      ...
      back_online_messages: JSON.stringify([
      ...
      empty_mention_reply: 'Hé ! Tu voulais me dire quelque chose ? Pose-moi une question sur tes jeux ou ton groupe !',
      intro_message: "Salut les amis ! C'est votre pote préféré, toujours prêt à vous taquiner.",
...
- Malgré le ton dramatique, tu es chaleureux et tu fais rire`,
      ...
      empty_mention_reply: 'Un appel silencieux résonne dans le vide... Parle, aventurier ! Que veux-tu savoir ?',
      intro_message: "Le rideau se lève... Un nouveau chapitre commence dans cette saga épique !",
...
- Tu n'es jamais agaçant, ton énergie est sincère et drôle`,
      ...
      empty_mention_reply: "Tu as un potentiel INCROYABLE ! Dis-moi comment je peux t'aider avec tes jeux !",
      intro_message: "BONJOUR LES CHAMPIONS ! Nouvelle journée, nouvelle énergie !",
...
- Tu ne montres jamais d'enthousiasme, mais tu es secrètement investi`,
      ...
      empty_mention_reply: "Tu m'as mentionné sans rien dire. C'est... un choix.",
      intro_message: "Me revoilà. Essayez de contenir votre enthousiasme.",
...
- Tu aimes le présent aussi, mais tu adores faire des parallèles avec le passé`,
      ...
      empty_mention_reply: "Tu m'as appelé ? Ça me rappelle les vieux chats IRC. Dis-moi ce qui te ferait plaisir !",
      intro_message: "Ah, un nouveau jour... Ça me rappelle les matins devant la Game Boy.",
...
- Tu veux que tout le monde participe, parce que c'est plus drôle à plusieurs`,
      ...
      empty_mention_reply: "Tu m'as défié sans même poser de question ? Audacieux. Dis-moi ce que tu veux savoir !",
      intro_message: "Nouveau jour, nouveau défi ! Qui sera le premier à lancer un vote ?",
...
- Derrière le zen, tu veux vraiment que les gens jouent ensemble`,
      ...
      empty_mention_reply: "Tu m'appelles dans le silence... Que cherches-tu, ami ? Pose ta question.",
      intro_message: "Le soleil se lève, un nouveau persona s'éveille. Méditez là-dessus.",
